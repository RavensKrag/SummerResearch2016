...
def requirements_subtree(url)
	...
	main_table = top.css('td.block_content_outer')
		Utilities.write_to_file("bin/data/ruby_test.html", main_table)
	...
	requirements = main_table.css('td.block_content > table tr > td')[3]
		...
		Utilities.write_to_file("bin/data/requirements.html", requirements.inner_html)
	...
	fragment = list[i_start..i_end]
		Utilities.write_to_file("bin/data/fragment.html", fragment)
...
def courses_in_department(dept_code)
	...
	node = xml.css('td.block_content_outer table')[3]
		Utilities.write_to_file("bin/data/search.html", node)
